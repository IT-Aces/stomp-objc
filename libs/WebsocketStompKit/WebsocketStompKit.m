...
- (void)connectWithLogin:(NSString *)login
    ...
    if (login && passcode) {
        headers = @{kHeaderLogin: login, kHeaderPasscode: passcode};
    }
...
- (void)checkPong:(NSTimer *)timer  {
    ...
    double delta = [lastServerResponse timeIntervalSinceNow];
    if (fabs(delta) > (ttl * 2)) {
...
- (void)websocket:(JFRWebSocket*)socket didReceiveData:(NSData*)data {
    ...
    lastServerResponse = [NSDate date];
